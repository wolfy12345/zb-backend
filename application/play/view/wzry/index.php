<?php echo $this->renderCommon('header_content');?>
    ...
    <div id="page1" class="box active">
        <div id="openexample" class="gameinfobox">
            <img src="<?php echo $this->baseUrl;?>/example/wzry/show.jpg">
            <div class="gamebox">
                <div class="game">
                    <dl class="meta">
    ...
    <script>
        ...
        //头像
        $.ajax({
            url: "?c=upload&a=remote&url=<?php echo $_SESSION['headimgurl']?$_SESSION['headimgurl']:'';?>&openid=<?php echo $_SESSION['openid']?$_SESSION['openid']:'';?>",
            type: "get",
            processData: false,
            contentType: false,
            dataType: 'json',
    ...
    <script>
        $(function () {
            $('#submitbtn').click(function(){
                ...
                $('.footer-more').hide();
                $('.footer-ad2').hide();
                $('#result').attr('src', "?m=play&c=<?php echo $this->ctl;?>&a=image&name="+name+"&avatar="+avatar+"&type="+type);
                $('#savetipsbox').show();
